    ...
    stages {
            ...
            steps {
                script {
                    bat """
                        ...
                        wsl git remote set-url origin https://%DOCKERHUB_USERNAME%:%GITHUB_TOKEN%@github.com/ben14132-01/java-app-10.git
            ...
            steps {
                script {
                    def response = bat(
                        ...
                        curl -X POST ^
                        -u admin:admin ^
                        "http://localhost:9001/api/projects/create?project=java-app-10&name=java-app-10"
                        ''',
                        returnStatus: true
                    )
        ...
        stage('SonarQube Analysis') {
            steps {
                dir('src/SpringAPI') {
                    bat '''
                    mvn clean verify sonar:sonar "-Dsonar.projectKey=java-app-10" "-Dsonar.projectName='java-app-10'" "-Dsonar.host.url=http://localhost:9001" "-Dsonar.token=%SONAR_TOKEN%"
                    '''
                }
            }
        }
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    def appName = 'java-app-10' //need to change appname
                    bat """
        ...
        stage('Deploy with Helm') {
            steps {
                script {
                    ...
                    bat """
                        wsl helm upgrade --install java-app ./charts/java-app-10 -n java --create-namespace --set image.tag=${commitId}
                    """
                }
            }
        }
